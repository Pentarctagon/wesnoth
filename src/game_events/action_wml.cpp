...
WML_HANDLER_FUNCTION(replace_map,, cfg)
{
	...
	try {
		...
		lg::log_to_chat() << "replace_map: Unable to load map " << log_map_name << std::endl;
	...
	|| map.total_height() > game_map->total_height()) {
		if (!cfg["expand"].to_bool()) {
			lg::log_to_chat() << "replace_map: Map dimension(s) increase but expand is not set" << std::endl;
	...
	|| map.total_height() < game_map->total_height()) {
		if (!cfg["shrink"].to_bool()) {
			lg::log_to_chat() << "replace_map: Map dimension(s) decrease but shrink is not set" << std::endl;
	...
	if (errmsg) {
		lg::log_to_chat() << *errmsg << std::endl;
		ERR_WML << *errmsg;
	}
